/*
  ...
  vcg export
  Copyright (C) 2002 Hua Bao (huabao_2001@yahoo.com)
...
int xbm_write_vcg_line(xbm_type x, FILE *fp, dcube *level_cube,dcube *edge_cube, int st1, int st2)
{
  int i;
  int pos_ref;
  char var_name[32];
  fprintf(fp, "edge: {");    
  fprintf(fp, "  sourcename:\" %s \"", xbm_GetStNameStr(x, st1));
  fprintf(fp, "  targetname:\" %s \"", xbm_GetStNameStr(x, st2));
...
int xbm_write_vcg_tr(xbm_type x, FILE *fp, int tr_pos)
{
  int i;
  ...
  return xbm_write_vcg_line(x, fp, level_cube, edge_cube, st1, st2);
  
}

int xbm_write_vcg_st(xbm_type x, FILE *fp, int st_pos)
{
  fprintf(fp," node: {");
  fprintf(fp,"  title:\"");
...
int xbm_WriteVCGFP(xbm_type x, FILE *fp)
{
  int i;
  
  fprintf(fp, "graph:  { title:\" CFG_GRAPH\"\n\n");
  fprintf(fp, " layoutalgorithm: dfs\n");
  fprintf(fp, " display_edge_labels: yes \n");
  fprintf(fp, " splines: yes\n");
  fprintf(fp, " port_sharing: no\n");
  fprintf(fp, " finetuning: no\n\n");
  
  fprintf(fp, " node.height:80\n");
  fprintf(fp, " node.width: 100\n");
  i = -1;
  while( xbm_LoopSt(x, &i) != 0 )
    if ( xbm_write_vcg_st(x, fp, i) == 0 )
      return 0;
      
      
  i = -1;
  while( xbm_LoopTr(x, &i) != 0 )
    if ( xbm_write_vcg_tr(x, fp, i) == 0 )
      return 0;
...
int xbm_WriteVCG(xbm_type x, const char *name)
{
  ...
  if ( fp == NULL )
    return 0;
  xbm_Log(x, 5, "XBM: Write vcg file '%s'.", name);
  ret = xbm_WriteVCGFP(x, fp);
  fclose(fp);
  return ret;
}
